# agents/agent_project_designer.py
...
class AgentProjectDesigner(BaseAgent):
    def __init__(self, nom="AgentProjectDesigner", role=None, memoire_persistante=None, verbose=False):
        ...
        self.memoire_persistante = memoire_persistante or LongTermMemory(
            ...
            description="Mémoire persistante de l'agent project_designer"
        )
        ...
        if self.verbose:
            print(f"[{self.name} INIT] Agent ProjectDesigner initialisé en mode verbeux.")
...
5. le code écrit est en python, sauf consigne contraire explicite.
